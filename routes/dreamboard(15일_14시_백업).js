...
async function uploadDataToGCS(file, data, bucketName, destFileName) {
  try {
    console.log(data, "중간 로그 1:");
    // GCS에 데이터를 저장(업로드)하는 부분
    await file.save(data, {
        // 업로드될 데이터의 메타데이터 설정
        metadata: {
            // 데이터의 MIME 타입 설정. 여기서는 텍스트 데이터로 가정합니다.
            // 필요에 따라 MIME 타입을 변경할 수 있습니다.
            contentType: 'text/plain'
        }
    });
    console.log(`파일 ${bucketName}에 ${destFileName} 업로드`); // 중간 로그
  } catch (error) {
...
router.post("/add", async function(req, res){
  ...
  const destFileName = `${req.session.user.user_id}_example.txt`; // ${req.session.user.user_id} + 제목
  // 버킷 및 파일 참조 생성
  ...
  try {
    await uploadDataToGCS(file, value, bucketName, destFileName);
