...
(function () {
  ...
  var selectionSettings = function () {
    const _self = this;
    $(`${this.containerSelector} .main-panel`).append(`<div class="toolpanel" id="select-panel"><div class="content"><p class="title">설정</p></div></div>`);
        ...
        <div class="text-section">
          ...
          <div class="family">
            ...
            <select id="font-family">
              ...
              <option value="'Myeongjo A1', serif">명조 A1</option>
              <option value="'Jeju Gothic', sans-serif">제주 고딕</option>
              <option value="'Jeju Myeongjo', serif">제주 명조</option>
              <option value="'Do Hyeon', sans-serif">도현</option>
              <option value="'Black Han Sans', sans-serif">블랙한 산스</option>
              <option value="'Hi Melody', cursive">하이 멜로디</option>
          ...
          <div class="sizes">
            <div class="input-container"><label>글자 크기</label>
            ...
            <div class="input-container"><label>줄간격</label>
              <div class="custom-number-input">
              <button class="decrease">-</button>
            ...
            <div class="input-container"><label>자간</label>
              <div class="custom-number-input">
              <button class="decrease">-</button>
    ...
    (() => {
      ...
      //`styleButtons` 배열에 있는 각 버튼 요소에 대해 반복
      styleButtons.forEach(button => {
        // 현재 반복 중인 버튼 요소의 `id` 속성 값을 가져옴
        const id = button.id;
        // 버튼의 `id` 값을 기반으로 툴팁에 표시될 텍스트 가져와서 `tooptipText` 변수에 저장
        const tooltipText = {
          bold: '굵게',
          italic: '기울임꼴',
          underline: '밑줄',
        ...
        // 새 `div`요소 생성하여 툴팁 역할을 할 요소 만듬
        const tooltip = document.createElement('div');
        // 생성한 `div` 요소의 클래스 이름을 `button-tooltip`으로 설정, 툴팁 스타일을 적용할 클래스 할당
        tooltip.className = 'button-tooltip';
        // 생성한 `div` 요소의 내용을 앞서 가져온 `tooltipText`로 설저하여 툴팁에 표시될 텍스트를 삽입
        tooltip.textContent = tooltipText;
        // 버튼 요소에 방금 생성한 툴팁 요소 추가, 버튼의 하위 요소로 툴팁 삽입
        button.appendChild(tooltip);


        // 버튼에 마우스 올가갈 때 실행될 함수 등록
        button.addEventListener('mouseenter', () => {
          // 툴팁의 `display` 속성을 `block`으로 설정하여 마우스 버튼 위에 올라갔을 때 툴팁 표시
          tooltip.style.display = 'block';
        });
        // 버튼에서 마우스가 떠날 때 실행될 함수 등록
        button.addEventListener('mouseleave', () => {
          // 마우스가 버튼 벗어날 때 툴팁 사라짐
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
        <div class="border-section">
          <h4>테두리</h4>
          <div class="input-container"><label>너비</label>
            <div class="custom-number-input">
            <button class="decrease">-</button>
          ...
          <div class="input-container"><label>색상</label><input id="color-picker" value="black"></div>
          <div class="input-container"><label>스타일</label><select id="input-border-style">${BorderStyleList.map(item => `<option value='${JSON.stringify(item.value)}'>${item.label}</option>`)}</select></div>
          <div class="input-container"><label>모서리</label><select id="input-corner-type"><option value="miter" selected>사각</option><option value="round">둥글게</option></select></div>
          <hr>
        </div>
      `);
    ...
    (() => {
      let alignmentButtonsHtml = '';
      AlignmentButtonList.forEach(item => {
        alignmentButtonsHtml += `<button class="alignment-button" data-pos="${item.pos}">${item.icon}</button>`;
      });
      ...
      const alignmentButtons = document.querySelectorAll(`${this.containerSelector} .toolpanel#select-panel .alignment-section .alignment-button`);
    ...
    (() => {
      ...
      (() => {
        ...
        const addButtonEventHandlers = (buttonId, clickHandler) => {
          const button = $(`${containerSelector} .toolpanel#select-panel .object-options #${buttonId}`);

          // Add hover functionality
          const tooltip = document.createElement('div');
          tooltip.className = 'button-tooltip';
          tooltip.textContent = buttonId;
        ...
        addButtonEventHandlers('가로대칭', () => {
          // Add your flip-h click handler here
        });

        addButtonEventHandlers('세로대칭', () => {
          // Add your flip-v click handler here
        });

        addButtonEventHandlers('앞으로', () => {
        });
      
        addButtonEventHandlers('뒤로', () => {
        });

        addButtonEventHandlers('맨앞으로', () => {
        });
      
        addButtonEventHandlers('맨뒤로', () => {
        });

        addButtonEventHandlers('복제', () => {
          // Add your duplicate click handler here
        });

        addButtonEventHandlers('삭제', () => {
          // Add your delete click handler here
        });

        addButtonEventHandlers('그룹화', () => {
          // Add your group click handler here
        });

        addButtonEventHandlers('그룹해제', () => {
          // Add your ungroup click handler here
        });
      })();

      $(`${this.containerSelector} .toolpanel#select-panel .object-options #가로대칭`).click(() => {
        this.activeSelection.set('flipX', !this.activeSelection.flipX);
        this.canvas.renderAll(), this.canvas.trigger('object:modified');
      })
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #세로대칭`).click(() => {
        this.activeSelection.set('flipY', !this.activeSelection.flipY);
        this.canvas.renderAll(), this.canvas.trigger('object:modified');
      })
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #앞으로`).click(() => {
        this.canvas.bringForward(this.activeSelection)
        this.canvas.renderAll(), this.canvas.trigger('object:modified');
      })
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #뒤로`).click(() => {
...
// 맨 앞으로 보내기 버튼 클릭 이벤트 처리
$(`${this.containerSelector} .toolpanel#select-panel #맨앞으로`).click(function () {
  bringToFront(_self.canvas, _self.activeSelection);
});

// 맨 뒤로 보내기 버튼 클릭 이벤트 처리
$(`${this.containerSelector} .toolpanel#select-panel #맨뒤로`).click(function () {
  sendToBack(_self.canvas, _self.activeSelection);
});

// 맨 앞으로 보내기 기능 구현
const bringToFront = (canvas, object) => {
...
// 맨 뒤로 보내기 기능 구현
const sendToBack = (canvas, object) => {
  canvas.sendToBack(object);
  canvas.renderAll();
  canvas.trigger('object:modified');
};
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #복제`).click(() => {
        let clonedObjects = []
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #삭제`).click(() => {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #그룹화`).click(() => {
        if (this.activeSelection.type !== 'activeSelection') return;
        this.canvas.getActiveObject().toGroup()
        this.canvas.requestRenderAll(), this.canvas.trigger('object:modified')
      })
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #그룹해제`).click(() => {
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
        <div class="effect-section">
          ...
          <div class="input-container"><label>흐림</label><input class="effect" id="blur" type="range" min="0" max="100" value="50"></div>
          <div class="input-container"><label>밝기</label><input class="effect" id="brightness" type="range" min="0" max="100" value="50"></div>
          <div class="input-container"><label>채도</label><input class="effect" id="saturation" type="range" min="0" max="100" value="50"></div>
          <h5>감마값</h5>
